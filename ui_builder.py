...
class UIBuilderMixin:
    ...
    def _build_ui(self):
        ...
        # Templates header
        ttk.Label(left, text="Templates", font=("Segoe UI", 10, "bold")).pack(anchor="w", padx=8, pady=(8, 0))
        tpl_frame = ttk.Frame(left)
        self._tut_tpl_frame = tpl_frame
        tpl_frame.pack(fill="both", expand=True, padx=8, pady=(2, 4))
        tpl_sb = ttk.Scrollbar(tpl_frame, orient="vertical")
        self.template_list = tk.Listbox(tpl_frame, height=8, yscrollcommand=tpl_sb.set)
        ...
        ttk.Button(btns, text="New", command=self.new_template).pack(side="left", fill="x", expand=True, padx=1)
        ttk.Button(btns, text="Duplicate", command=self.duplicate_template).pack(side="left", fill="x", expand=True, padx=1)
        ttk.Button(btns, text="Delete", command=self.delete_template).pack(side="left", fill="x", expand=True, padx=1)
        ...
        self._tut_save_template_btn.pack(fill="x", padx=8, pady=1)
        ttk.Button(left, text="Save All", command=self.save_all).pack(fill="x", padx=8, pady=1)
        ttk.Button(left, text="Import Template from CSV Row(s)", command=self.import_from_csv_rows_open_tabs).pack(fill="x", padx=8, pady=1)
        chk = ttk.Frame(left)
        chk.pack(fill="x", padx=8, pady=2)
        ttk.Button(chk, text="Check All", command=self.check_all_current_tab).pack(side="left", fill="x", expand=True, padx=1)
        ttk.Button(chk, text="Uncheck All", command=self.uncheck_all_current_tab).pack(side="left", fill="x", expand=True, padx=1)
        self.collapse_btn = ttk.Button(left, text="Collapse Unincluded Fields", command=self.toggle_collapse_current_tab)
        self.collapse_btn.pack(fill="x", padx=8, pady=2)
        ttk.Separator(left, orient="horizontal").pack(fill="x", padx=8, pady=4)
        ttk.Button(left, text="Import CSV rows into current tab", command=self.import_rows_into_current_tab).pack(fill="x", padx=8, pady=1)
        # Jira
        ttk.Separator(left, orient="horizontal").pack(fill="x", padx=8, pady=4)
        ttk.Label(left, text="Jira", font=("Segoe UI", 10, "bold")).pack(anchor="w", padx=8, pady=(2, 0))
        jira_frame = ttk.Frame(left)
        jira_frame.pack(fill="x", padx=8, pady=3)
        self._tut_set_jira_btn = ttk.Button(jira_frame, text="Set Jira API...", command=self.set_jira_credentials)
        self._tut_set_jira_btn.pack(fill="x", pady=1)
        ttk.Button(jira_frame, text="Test Connection", command=self.test_jira_connection).pack(fill="x", pady=1)
        self._tut_fetch_btn = ttk.Button(jira_frame, text="Fetch My Issues...", command=self.fetch_my_issues_dialog)
        self._tut_fetch_btn.pack(fill="x", pady=1)
        ttk.Button(jira_frame, text="Auto-Fetch Settings...", command=self.auto_fetch_settings_dialog).pack(fill="x", pady=1)
        ttk.Button(jira_frame, text="Refresh Fetched Tickets", command=self.refresh_fetched_tickets).pack(fill="x", pady=1)
        self._tut_config_btn = ttk.Button(jira_frame, text="Configure Reminders...", command=self.configure_reminders_dialog)
        self._tut_config_btn.pack(fill="x", pady=1)
        ttk.Separator(jira_frame, orient="horizontal").pack(fill="x", pady=3)
        ttk.Button(jira_frame, text="Export Bundle…",
                   command=self._share_bundle_dialog).pack(fill="x", pady=1)
        ttk.Button(jira_frame, text="Import Bundle…",
                   command=self._import_bundle_dialog).pack(fill="x", pady=1)
        # Help
        ttk.Separator(left, orient="horizontal").pack(fill="x", padx=8, pady=4)
        ttk.Label(left, text="Help", font=("Segoe UI", 10, "bold")).pack(anchor="w", padx=8, pady=(2, 0))
        help_frame = ttk.Frame(left)
        help_frame.pack(fill="x", padx=8, pady=3)
        ttk.Button(help_frame, text="Show Tutorial", command=lambda: self._run_tutorial(force=True)).pack(fill="x", pady=1)
        ttk.Button(help_frame, text="Check for Updates",
                   command=lambda: self._check_for_updates(manual=True)).pack(fill="x", pady=1)
        chan_row = ttk.Frame(help_frame)
        chan_row.pack(fill="x", pady=(2, 0))
        ...
        for ch_val, ch_lbl in [("stable", "Stable"), ("experimental", "Experimental")]:
            ttk.Radiobutton(chan_row, text=ch_lbl, variable=_chan_var,
                            value=ch_val).pack(side="left", padx=(4, 0))
        from config import APP_VERSION as _v
        ttk.Label(help_frame, text=f"v{_v}", foreground="#888888",
                  font=("Segoe UI", 8)).pack(anchor="e", pady=(1, 0))
        # Data
        ttk.Separator(left, orient="horizontal").pack(fill="x", padx=8, pady=4)
        ttk.Label(left, text="Data", font=("Segoe UI", 10, "bold")).pack(anchor="w", padx=8, pady=(2, 0))
        data_frame = ttk.Frame(left)
        data_frame.pack(fill="x", padx=8, pady=3)
        ...
        path_entry.pack(fill="x", pady=(1, 2))
        ttk.Button(data_frame, text="Load Templates from File…",
                   command=self._browse_and_load_templates).pack(fill="x", pady=1)
        ttk.Button(data_frame, text="Reload Current File",
                   command=self._reload_templates_file).pack(fill="x", pady=1)
        # Bundle
        ttk.Separator(left, orient="horizontal").pack(fill="x", padx=8, pady=4)
        ttk.Label(left, text="Bundle", font=("Segoe UI", 10, "bold")).pack(anchor="w", padx=8, pady=(2, 0))
        bundle_frame = ttk.Frame(left)
        bundle_frame.pack(fill="both", expand=True, padx=8, pady=3)
        ...
        self.bundle_listbox = tk.Listbox(bnd_inner, height=6, yscrollcommand=bnd_sb.set)
        ...
        bbtn_frame.pack(side="right", fill="y", padx=(4, 0))
        self._tut_add_bundle_btn = ttk.Button(bbtn_frame, text="Add ▶", command=self.add_active_tab_to_bundle)
        self._tut_add_bundle_btn.pack(fill="x", pady=1)
        ttk.Button(bbtn_frame, text="Jira",   command=self._open_selected_bundle_in_jira).pack(fill="x", pady=1)
        ttk.Button(bbtn_frame, text="Export", command=self._share_bundle_dialog).pack(fill="x", pady=1)
        ttk.Button(bbtn_frame, text="Remove", command=self.remove_selected_from_bundle).pack(fill="x", pady=1)
        ttk.Button(bbtn_frame, text="Clear", command=self.clear_bundle).pack(fill="x", pady=1)
        ttk.Button(bbtn_frame, text="Rename", command=self.rename_bundle).pack(fill="x", pady=1)
        bundle_export_frame = ttk.Frame(left)
        bundle_export_frame.pack(fill="x", padx=8, pady=3)
        self._tut_upload_btn = ttk.Button(bundle_export_frame, text="Upload Bundle to Jira...", command=self.upload_bundle_to_jira_dialog)
        self._tut_upload_btn.pack(fill="x", pady=3)
        self._tut_bulk_import_btn = ttk.Button(bundle_export_frame, text="Bulk Import...", command=self.bulk_import_dialog)
        self._tut_bulk_import_btn.pack(fill="x", pady=1)
        # RIGHT
